...
class Upload
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->savePath = isset($config['savePath']) ? $config['savePath'] : '/public/upload/attach';
        $this->urlPath = isset($config['urlPath']) ? $config['urlPath'] : '/upload/attach';
    ...
    private function file($file)
    {
        if (isset($file['error']) && $file['error'] != 0) {
            ...
            if ($this->autoName)
                ...
                $filename = $file['name'];
            $filePath = Path::get() . '/' . $saveDir . $filename;
